...
class Agent:
    ...
    def Solve(self, problem):
        ...
        sameIncr = 0.01  # same general property increment value
        ...
        if (problem.problemType=="2x2"):
            ...
            for x in range(1, 7):     # loop through each potential answer from 1 to 6:
                ...
                for ansObjName in self.sortDict(problem.figures[str(x)].objects):
                    ...
                    tot = self.compareObjToAns(ansObj, c, transformDict, tot)
                    # # compare A to C
        ...
        # check if problem type is 3x3
            ...
            for x in range(1, 9):     # loop through each potential answer from 1 to 6:
                ...
                for ansObjName in self.sortDict(problem.figures[str(x)].objects):
                    ...
                    self.compareAtoObj(a, b, transformDict)
                    
                    # compare C to D

                    self.compareObjToAns(ansObj, c, transformDict)
                    
                    # # compare A to C
                    
                    self.compareAtoObj(a, c, transformDict)

                    # # compare B to D

                    self.compareObjToAns(ansObj, b, transformDict)
    ...
    def compareAtoObj(self, a, b, transformDict, tot):
        makeAngTransCompAB = False
        for aObjName in self.sortDict(a.objects):
            aObj = a.objects[aObjName]
            for bObjName in self.sortDict(b.objects):
    ...
    def compareObjToAns(self, ansObj, c, transformDict, tot, fillIncr):
        for cObjName in self.sortDict(c.objects):
            makeAngTransCompCD = False
            cObj = c.objects[cObjName]
            ...
            if 'alignment' in cObj.attributes and 'alignment' in ansObj.attributes:
                if cObj.attributes['alignment'] != ansObj.attributes['alignment']:
                    ...
                    print('cdAlignDiff: ' + transformDict['cdAlignDiff'])
            # calculate alignment transform differences
            if sorted(transformDict['abAlignDiff']) == sorted(transformDict['cdAlignDiff']) and transformDict[
                'abAlignDiff'] != 'empty-' and transformDict['cdAlignDiff'] != 'empty-':
                tot += alignIncr
